...
PyObject *AmiPyIsolation_GetDict( const char *ctx )
{
	...
	if( lock.Lock(1000) )
	{
		...
		if( !exists )
		{
			...
			if( !builtins || PyDict_SetItemString( pDict, "__builtins__", builtins ) < 0 )
			{
		...
		else gLogger( "found dict (key=\"" + sKey + "\")", Logger::MSG_DEBUG );
...
void AmiPyIsolation_ClearAll()
{
	...
	if( lock.Lock(5000) ) // wait max 5s
	{
		...
		if( bInitialzied )
		{
			...
			gLogger( "all dicts released (" + t.getAsString() + ")", Logger::MSG_MAIN_EVENTS );
		}

		lock.Unlock();
	}
	else PRINT_ERROR( "could not lock isolation environment during destruction" );
}
