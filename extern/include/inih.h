...
static inline char to_lower_char(char c) { return (char)tolower((int)c); }

inline bool INIReader::GetBoolean(std::string section, std::string name,
                                  bool default_value) const {
  std::string valstr = Get(section, name, "");
  // Convert to lower case to make string comparisons case-insensitive
  std::transform(valstr.begin(), valstr.end(), valstr.begin(), to_lower_char);
...
inline std::string INIReader::MakeKey(std::string section, std::string name) {
  ...
  std::transform(key.begin(), key.end(), key.begin(), to_lower_char);
  return key;
}
