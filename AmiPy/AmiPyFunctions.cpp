...
static CStringA AmiPyPythonExceptionOnlyToString( PyObject *type, PyObject *value, PyObject *tracebackModule )
{
	assert( type );
	assert( value );
	assert( tracebackModule );

	if( !value || !type || !tracebackModule ) return "";

	PyObject *format_exception_only = PyObject_GetAttrString( tracebackModule, "format_exception_only" );

	if( !format_exception_only || !PyCallable_Check( format_exception_only ) )
	{
		if( format_exception_only ) Py_DecRef( format_exception_only );
		return "* traceback module does not have format_exception_only function: "
				"cannot acquire exception info\n";
	}

	PyObject *ExcStrs = PyObject_CallFunctionObjArgs( format_exception_only, type, value, NULL );
	Py_DecRef( format_exception_only );

	if( !ExcStrs || !PyList_Check( ExcStrs ) )
	{
		if( ExcStrs ) Py_DecRef( ExcStrs );
	...
	for( int i = 0; i < siz; i++ )
	{
		...
		if( !cur ) ErrMsg += "* cannot get exception info string\n";
		else ErrMsg += CStringA() + "* " + ASCIICapsule::FromObject( cur, false ).Data();
	}
...
static CStringA AmiPyPythonCallstackToString( PyObject *traceback, PyObject *tracebackModule )
{
	assert( traceback );
	assert( tracebackModule );

	if( !traceback || !tracebackModule ) return "";
	...
	{
		if( extract_tb ) Py_DecRef( extract_tb );
		
		return TbMsg + "* * traceback module does not have extract_tb function: "
				"cannot acquire traceback info\n";
	}

	PyObject *tracebackList = PyObject_CallFunctionObjArgs( extract_tb, traceback, NULL );
	PyObject *FrameSummaryType = PyObject_GetAttrString( tracebackModule, "FrameSummary" );
	Py_DecRef( extract_tb );

	if( !tracebackList || !PyList_Check( tracebackList ) ||
		!FrameSummaryType || !PyType_Check( FrameSummaryType ) 
		)
	{
		if( tracebackList ) Py_DecRef( tracebackList );
		if( FrameSummaryType ) Py_DecRef( FrameSummaryType );
	...
	for( Py_ssize_t i = siz; i--;)
	{
		...
		if( cur && PyType_IsSubtype( Py_TYPE( cur ), (PyTypeObject *)FrameSummaryType ) )
		{
			...
			if( filename && PyUnicode_Check( filename ) &&
				lineno && PyLong_Check( lineno ) &&
				name && PyUnicode_Check( name ) &&
				line && PyUnicode_Check( line )
				)
			{
				TbMsg.AppendFormat(
					"* * File: '%s', line %-3d (in %s)\n",
				...
				if( line != Py_None ) TbMsg +=
					CStringA() + "* * * " + ASCIICapsule::FromUnicode( line, false ).Data() + "\n";
			}
			else TbMsg += "* * Invalid FrameSummary: filename/lineno/name/line is NULL or is not expected type\n";
		...
		else TbMsg += "* * traceback item is not FrameSummary\n";
	}
...
{
	if( !PyErr_Occurred() ) return "* Python error did not occur\n";
	...
	if( !tracebackModule ) ErrMsg += "* traceback module not avaiable: cannot acquire exception info\n";
	else
	{
		ErrMsg += AmiPyPythonExceptionOnlyToString( type, value, tracebackModule );

		if( traceback ) ErrMsg += AmiPyPythonCallstackToString( traceback, tracebackModule );
...
{
	if( !EnsureInitialized() ) return AmiVar{ VAR_FLOAT, EMPTY_VAL };

	if( iNumArgs < 2 || pArgs[0].type != VAR_STRING || pArgs[1].type != VAR_STRING )
	{
		PRINT_ERROR( "First argument is context name and second is function name" );
		return AmiVar{ VAR_FLOAT, EMPTY_VAL };
	}

	gLogger( CStringA() + "Start PyEvalFunction( " +
		"\"" + pArgs[0].string + "\", " +
		"\"" + pArgs[1].string + "\", ... )",
		Logger::MSG_FN_START );
...

	{
		...
		if( !pGlobals ) return AmiVar{ VAR_FLOAT, EMPTY_VAL };

		// Find function
		PyObject *pFunction = PyDict_GetItemString( pGlobals, pArgs[1].string ); // Borrowed ref.

		if( pFunction == NULL || !PyCallable_Check( pFunction ) )
		{
			PRINT_ERROR( "'%s' does not exist in global scope OR is not callable (not a function)", pArgs[1].string );
			return AmiVar{ VAR_FLOAT, EMPTY_VAL };
		}
		...
		for( int j = 0; j < iNumArgs - 2; j++ )
		{
			...
			if( !pObj )
			{
				...
				if( PyErr_Occurred() )
					AmiPyPrintError( "Error occured during AFL to Py argument conversion:" );
...
AmiVar AmiPyLoadFromFile( int iNumArgs, AmiVar *pArgs )
{
	...
	gLogger( CStringA() + "Start PyLoadFromFile( " + 
		"\"" + pArgs[0].string + "\", " +
		"\"" + pArgs[1].string + "\" )"
		, Logger::MSG_FN_START );
...

	{
		...
		if( PyErr_Occurred() ) AmiPyPrintError( "Error occured during Python execution:" );
	}
