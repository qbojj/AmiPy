...
struct PackedDateFormat4 {
  unsigned int
      Tick : 4; // 0..11 - Tick is 5 seconds   15 is reserved as EOD marker
  unsigned int
      Minute : 6; // 0..59                       63 is reserved as EOD marker
  unsigned int
      Hour : 5; // 0..23                       31 is reserved as EOD marker
...
#ifdef _M_X64
  char WebID[8]; // ON 64 BIT PLATFORM THIS HAS TO BE 8 due to alignment
                 // requirements
#else
...
struct _IntradaySettingsFormat4 {
  ...
  unsigned int SessionStart; // bit encoding HHHHH.MMMMMM.0000	 hours << 10 | (
                             // minutes << 4 )
...
struct _WorkspaceOldFormat4 {
  ...
  int SessionStart; // bit encoding HHHHH.MMMMMM.0000	 hours << 10 | ( minutes
                    // << 4 )
  int SessionEnd;   // 			  //
  BOOL FilterWeekends;
};
...
///////////////////////////////
//  Quote format conversion functions (old (format4) 32 bit date/time <-> new
//  (format5), 64 bit date/time )
///////////////////////////////
